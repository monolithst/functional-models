...
describe('/src/validation.js', () => {
  ...
  describe('#createPropertyValidate()', () => {
    it('should result in {}.functions.validateMyProperty when key="myProperty"', () => {
      const actual = createPropertyValidate('myProperty', {})('value')
      assert.isOk(actual.functions.validateMyProperty)
    })
    it('should create a isRequired validator when config contains isRequired=true', async () => {
      const property = createPropertyValidate('myProperty', { required: true})(null)
      const actual = (await property.functions.validateMyProperty()).length
      const expected = 1
      assert.equal(actual, expected)
    })
    it('should use the validators passed in', async () => {
      const property = createPropertyValidate('myProperty', { validators: [
        maxTextLength(5)
      ]})("hello world")
      const actual = (await property.functions.validateMyProperty()).length
      const expected = 1
